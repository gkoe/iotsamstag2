...
bool Actor::IsOnlyDigits(const char* text)
{
...
void Actor::syncState()
{
	...
	{
		snprintf(message,MESSAGE_LENGTH-1,"SyncState, acorState: %f, commandValue: %f", actorState, _commandValue);
		Serial.printf("*AC %s\n",message);
		setActorTo(_commandValue);
		char fullTopic[100];
		sprintf(fullTopic, "%s/%s/state", Thing.getName(), _name);
		char payload[128];
		actorState = readStateFromActor();  // neu gesetzten Zustand holen
