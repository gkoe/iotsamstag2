...
//static const char ntpServerName[] = "10.0.0.51";
...
WiFiUDP Udp;
unsigned int localPort = 8888;  // local port to listen for UDP packets normally 123
...
time_t ThingTimeClass::getNtpTime()
{
	...
	Serial.print(F("*NT: Transmit NTP Request to "));
	// get a random server from the pool
	WiFi.hostByName(ntpServerName, ntpServerIP);
	//ntpServerIP.fromString(ntpServerName);
	...
	Serial.printf("!NT: No NTP Response :-(, packetsize: %d\n",size);
...
void ThingTimeClass::setNtpTimeSubscriber() {
	...
	Serial.println(F("*NT waiting for sync"));
